...
RULE_JSON_FILE = "published_rules/CG0117/CG0117.json"
DATASET_PATH = "json_datasets/CG0117/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['dm.xpt', 'Demographics', '.', 'IGNORE_TIMESTAMP', 0, 17], "Mismatch in 'Dataset Details', row 0"

    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[1].tolist() == ['ta.xpt', 'Trial Arms', '.', 'IGNORE_TIMESTAMP', 0, 10], "Mismatch in 'Dataset Details', row 1"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000367', 1, 'CG0117', None, 'ACTARM not in TA.ARM, but ACTARM does not equal Screen Failure, Not Assigned, Not Treated, or Unplanned Treatment in DM', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
