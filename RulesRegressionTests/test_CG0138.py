...
RULE_JSON_FILE = "published_rules/CG0138/CG0138.json"
DATASET_PATH = "json_datasets/CG0138/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['suppec.xpt', 'Supplemental Qualifiers for Exposure', '.', 'IGNORE_TIMESTAMP', 0, 2], "Mismatch in 'Dataset Details', row 0"

    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[1].tolist() == ['suppdm.xpt', 'Supplemental Qualifiers for Demographics', '.', 'IGNORE_TIMESTAMP', 0, 5], "Mismatch in 'Dataset Details', row 1"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000533', 1, 'CG0138', None, 'A SUPP domain other than SUPPDM contains population flags (name fragments reserved)', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
