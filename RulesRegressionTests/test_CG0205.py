...
RULE_JSON_FILE = "published_rules/CG0205/CG0205.json"
DATASET_PATH = "json_datasets/CG0205/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['sqapfamh.xpt', 'Supplemental Qualifiers for Findings About', '.', 'IGNORE_TIMESTAMP', 0, 3], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000293', 1, 'CG0205', None, 'Length of dataset name of SUPP-- dataset is greater than 8.', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
