...
RULE_JSON_FILE = "published_rules/CG0008/CG0008.json"
DATASET_PATH = "json_datasets/CG0008/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['ml.xpt', 'Meal Data', '.', 'IGNORE_TIMESTAMP', 0, 5], "Mismatch in 'Dataset Details', row 0"

    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[1].tolist() == ['pc.xpt', 'Pharmacokinetics Concentrations', '.', 'IGNORE_TIMESTAMP', 0, 5], "Mismatch in 'Dataset Details', row 1"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000028', 1, 'CG0008', None, '--TPTREF is empty and --ELTM is not empty', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
