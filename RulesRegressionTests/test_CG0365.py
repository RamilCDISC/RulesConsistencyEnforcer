...
RULE_JSON_FILE = "published_rules/CG0365/CG0365.json"
DATASET_PATH = "json_datasets/CG0365/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['aprelsub.xpt', 'Associated Persons Related to Subjects', '.', 'IGNORE_TIMESTAMP', 0, 6], "Mismatch in 'Dataset Details', row 0"

    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[1].tolist() == ['aplb.xpt', 'Associated Persons Laboratory Test Results', '.', 'IGNORE_TIMESTAMP', 0, 3], "Mismatch in 'Dataset Details', row 1"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000233', 1, 'CG0365', None, 'RDEVID must be empty when RSUBJID is populated and RSUBJID must be empty when RDEVID is populated', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
