...
RULE_JSON_FILE = "published_rules/CG0013/CG0013.json"
DATASET_PATH = "json_datasets/CG0013/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[1].tolist() == ['lb.xpt', 'Laboratory Test Results', '.', 'IGNORE_TIMESTAMP', 0, 9], "Mismatch in 'Dataset Details', row 1"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000550', 1, 'CG0013, TIG0298', None, 'Variable is not an allowed variable for an Observation Class', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
