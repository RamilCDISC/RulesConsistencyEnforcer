...
RULE_JSON_FILE = "published_rules/CG0124/CG0124.json"
DATASET_PATH = "json_datasets/CG0124/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['ta.xpt', 'Trial Arms', '.', 'IGNORE_TIMESTAMP', 0, 8], "Mismatch in 'Dataset Details', row 0"

    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[1].tolist() == ['dm.xpt', 'Demographics', '.', 'IGNORE_TIMESTAMP', 0, 12], "Mismatch in 'Dataset Details', row 1"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000331', 1, 'CG0124, CG0125', None, 'DM.ACTARMCD must equal SCRNFAIL, NOTASSGN, NOTTRT, or UNPLAN if the value is not present in TA.ARMCD', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
