...
RULE_JSON_FILE = "published_rules/CG0201/CG0201.json"
DATASET_PATH = "json_datasets/CG0201/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['relrec.xpt', 'Related Records', '.', 'IGNORE_TIMESTAMP', 0, 10], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000136', 1, 'CG0201', None, 'IDVARVAL and USUBJID are not completed, but IDVAR equals Sequence Number.', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
