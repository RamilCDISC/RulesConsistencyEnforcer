...
RULE_JSON_FILE = "published_rules/CG0191/CG0191.json"
DATASET_PATH = "json_datasets/CG0191/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['mb.xpt', 'Microbiology Specimen', '.', 'IGNORE_TIMESTAMP', 0, 2], "Mismatch in 'Dataset Details', row 0"

    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[1].tolist() == ['ms.xpt', 'Microbiology Susceptibility', '.', 'IGNORE_TIMESTAMP', 0, 10], "Mismatch in 'Dataset Details', row 1"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000188', 1, 'CG0191', None, 'The MB dataset is not present when the MS dataset is present.', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
