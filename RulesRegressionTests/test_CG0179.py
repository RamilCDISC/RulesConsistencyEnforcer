...
RULE_JSON_FILE = "published_rules/CG0179/CG0179.json"
DATASET_PATH = "json_datasets/CG0179/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['ie.xpt', 'Inclusion/Exclusion Criteria Not Met', '.', 'IGNORE_TIMESTAMP', 0, 3], "Mismatch in 'Dataset Details', row 0"

    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[1].tolist() == ['ti.xpt', 'Trial Inclusion/Exclusion Criteria', '.', 'IGNORE_TIMESTAMP', 0, 8], "Mismatch in 'Dataset Details', row 1"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000228', 1, 'CG0179', None, 'IETESTCD from IE domain does not exist in IETESTCD of TI domain.', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
