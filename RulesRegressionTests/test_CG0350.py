...
RULE_JSON_FILE = "published_rules/CG0350/CG0350.json"
DATASET_PATH = "json_datasets/CG0350/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['lb.xpt', 'Laboratory Test Results', '.', 'IGNORE_TIMESTAMP', 0, 10], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000219', 1, 'CG0350, SEND44, TIG0515', None, '--SCAT is equal to domain dataset label.', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
