...
RULE_JSON_FILE = "published_rules/CG0309/CG0309.json"
DATASET_PATH = "json_datasets/CG0309/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[2].tolist() == ['aprelsub.xpt', 'Associated Persons Relationships', '.', 'IGNORE_TIMESTAMP', 0, 1], "Mismatch in 'Dataset Details', row 2"

    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[3].tolist() == ['apmh.xpt', 'Associated Persons Medical History', '.', 'IGNORE_TIMESTAMP', 0, 4], "Mismatch in 'Dataset Details', row 3"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000181', 1, 'CG0309', None, 'AP-- Domain value length is not equal to 4.', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
