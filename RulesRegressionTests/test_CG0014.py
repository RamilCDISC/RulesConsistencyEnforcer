...
RULE_JSON_FILE = "published_rules/CG0014/CG0014.json"
DATASET_PATH = "json_datasets/CG0014/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[1].tolist() == ['ex.xpt', 'Exposure', '.', 'IGNORE_TIMESTAMP', 0, 55], "Mismatch in 'Dataset Details', row 1"

    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[2].tolist() == ['lb.xpt', 'Laboratory Test Results', '.', 'IGNORE_TIMESTAMP', 0, 46], "Mismatch in 'Dataset Details', row 2"

    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[3].tolist() == ['ta.xpt', 'Trial Arms', '.', 'IGNORE_TIMESTAMP', 0, 6], "Mismatch in 'Dataset Details', row 3"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000355', 1, 'CG0014, SEND12, TIG0057, TIG0299', None, 'At least one required variable is missing from dataset', 'SKIPPED'], "Mismatch in 'Rules Report', row 0"
