...
RULE_JSON_FILE = "published_rules/CG0352/CG0352.json"
DATASET_PATH = "json_datasets/CG0352/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['ft.xpt', 'Functional Tests', '.', 'IGNORE_TIMESTAMP', 0, 2], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000226', 1, 'CG0352', None, '--DTHREL must never be used in SDTM-based data for human clinical trials', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
