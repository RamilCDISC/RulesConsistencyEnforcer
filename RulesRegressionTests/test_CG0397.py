...
RULE_JSON_FILE = "published_rules/CG0397/CG0397.json"
DATASET_PATH = "json_datasets/CG0397/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['lb.xpt', 'Laboratory Test Results', '.', 'IGNORE_TIMESTAMP', 0, 4], "Mismatch in 'Dataset Details', row 0"

    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[1].tolist() == ['vs.xpt', 'Vital Signs', '.', 'IGNORE_TIMESTAMP', 0, 3], "Mismatch in 'Dataset Details', row 1"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000021', 1, 'CG0397, TIG0559', None, "--STRESC should not be blank when either --ORRES is provided or --DRVFL = 'Y' .", 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
