...
RULE_JSON_FILE = "published_rules/CG0140/CG0140.json"
DATASET_PATH = "json_datasets/CG0140/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['suppdm.xpt', 'Supplemental Qualifiers for DM', '.', 'IGNORE_TIMESTAMP', 0, 3], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000337', 1, 'CG0140', None, 'Multiple races are collected in SUPPDM but RACE in DM does not equal "MULTIPLE".', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
