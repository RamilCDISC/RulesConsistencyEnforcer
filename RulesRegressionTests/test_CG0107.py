...
RULE_JSON_FILE = "published_rules/CG0107/CG0107.json"
DATASET_PATH = "json_datasets/CG0107/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['ec.xpt', 'Exposure as Collected', '.', 'IGNORE_TIMESTAMP', 0, 4], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000292', 1, 'CG0107', None, 'EXVAMTU is present when EC domain exists.', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
