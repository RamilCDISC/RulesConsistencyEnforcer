...
RULE_JSON_FILE = "published_rules/CG0367/CG0367.json"
DATASET_PATH = "json_datasets/CG0367/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['apdm.xpt', 'Associated Persons Demographics', '.', 'IGNORE_TIMESTAMP', 0, 2], "Mismatch in 'Dataset Details', row 0"

    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[1].tolist() == ['dm.xpt', 'Demographics', '.', 'IGNORE_TIMESTAMP', 0, 2], "Mismatch in 'Dataset Details', row 1"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000235', 1, 'CG0367', None, 'RSUBJID must equal DM.USUBJID when RSUBJID is populated and RSUBJID does not equal POOLID', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
