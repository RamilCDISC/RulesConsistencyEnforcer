...
RULE_JSON_FILE = "published_rules/CG0045/CG0045.json"
DATASET_PATH = "json_datasets/CG0045/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['cm.xpt', 'Concomitant/Prior Medications', '.', 'IGNORE_TIMESTAMP', 0, 10], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000106', 1, 'CG0045', None, '--ENTPT is completed, but --ENRTPT is not completed.', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
