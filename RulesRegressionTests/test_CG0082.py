...
RULE_JSON_FILE = "published_rules/CG0082/CG0082.json"
DATASET_PATH = "json_datasets/CG0082/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['ae.xpt', 'Adverse Events', '.', 'IGNORE_TIMESTAMP', 0, 6], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000024', 1, 'CG0082', None, '--BODSYS is not empty and --BDSYCD is empty', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
