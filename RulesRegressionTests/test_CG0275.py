...
RULE_JSON_FILE = "published_rules/CG0275/CG0275.json"
DATASET_PATH = "json_datasets/CG0275/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['ts.xpt', 'Trial Summary', '.', 'IGNORE_TIMESTAMP', 0, 52], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000532', 1, 'CG0275', None, 'A record exists in TS where TSPARMCD = ADDON and TSVAL = Y but TSVAL for TSPARMCD = CURTRT is empty.', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
