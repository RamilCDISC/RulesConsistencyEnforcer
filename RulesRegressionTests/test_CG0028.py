...
RULE_JSON_FILE = "published_rules/CG0028/CG0028.json"
DATASET_PATH = "json_datasets/CG0028/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['ds.xpt', 'Disposition', '.', 'IGNORE_TIMESTAMP', 0, 14], "Mismatch in 'Dataset Details', row 0"

    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[1].tolist() == ['ss1.xpt', 'Subject Status1', '.', 'IGNORE_TIMESTAMP', 0, 14], "Mismatch in 'Dataset Details', row 1"

    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[2].tolist() == ['ss2.xpt', 'Subject Status2', '.', 'IGNORE_TIMESTAMP', 0, 14], "Mismatch in 'Dataset Details', row 2"

    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[3].tolist() == ['ae.xpt', 'Adverse Events', '.', 'IGNORE_TIMESTAMP', 0, 14], "Mismatch in 'Dataset Details', row 3"

    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[4].tolist() == ['lbae.xpt', 'Labs AE', '.', 'IGNORE_TIMESTAMP', 0, 14], "Mismatch in 'Dataset Details', row 4"

    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[5].tolist() == ['lbds.xpt', 'Labs DS', '.', 'IGNORE_TIMESTAMP', 0, 14], "Mismatch in 'Dataset Details', row 5"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000544', 1, 'CG0028, TIG0310', None, '--SEQ is not a unique number per USUBJID per domain, nor a unique number per POOLID per domain, including when the domain is split into multiple files', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
