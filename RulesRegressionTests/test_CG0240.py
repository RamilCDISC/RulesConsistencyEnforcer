...
RULE_JSON_FILE = "published_rules/CG0240/CG0240.json"
DATASET_PATH = "json_datasets/CG0240/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['ft.xpt', 'Functional Tests', '.', 'IGNORE_TIMESTAMP', 0, 4], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000141', 1, 'CG0240', None, '--TPT and --TPTNUM do not have a one-to-one relationship', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
