...
RULE_JSON_FILE = "published_rules/CG0409/CG0409.json"
DATASET_PATH = "json_datasets/CG0409/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[1].tolist() == ['dv.xpt', 'Protocol Deviations', '.', 'IGNORE_TIMESTAMP', 0, 1], "Mismatch in 'Dataset Details', row 1"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000172', 1, 'CG0409', None, 'STUDYID is not equal to DM.STUDYID', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
