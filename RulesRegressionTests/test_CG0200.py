...
RULE_JSON_FILE = "published_rules/CG0200/CG0200.json"
DATASET_PATH = "json_datasets/CG0200/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['relrec.xpt', 'Related Records', '.', 'IGNORE_TIMESTAMP', 0, 11], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000484', 1, 'CG0200', None, 'Unique RELID is NOT present on multiple RELREC records within a subject.', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
