...
COMMAND = (
    "python core.py test -s sdtmig -v 3.4 -r "
    + RULE_JSON_FILE
    + " -dp "
    + DATASET_PATH
    + ""
...
def test_excel():
    ...
    assert (
        list(new_df_dict["Dataset Details"].columns) == expected_columns
    ), "Column mismatch in 'Dataset Details'"

    assert clean_dataframe(new_df_dict["Dataset Details"]).iloc[0].tolist() == [
        "sv.xpt",
        "Subject Visits",
        ".",
        "IGNORE_TIMESTAMP",
        0,
        58,
    ], "Mismatch in 'Dataset Details', row 0"

    assert clean_dataframe(new_df_dict["Dataset Details"]).iloc[1].tolist() == [
        "tv.xpt",
        "Trial Visits ",
        ".",
        "IGNORE_TIMESTAMP",
        0,
        29,
    ], "Mismatch in 'Dataset Details', row 1"
    ...
    assert (
        list(new_df_dict["Issue Summary"].columns) == expected_columns
    ), "Column mismatch in 'Issue Summary'"
    ...
    assert (
        list(new_df_dict["Issue Details"].columns) == expected_columns
    ), "Column mismatch in 'Issue Details'"
    ...
    assert (
        list(new_df_dict["Rules Report"].columns) == expected_columns
    ), "Column mismatch in 'Rules Report'"

    assert clean_dataframe(new_df_dict["Rules Report"]).iloc[0].tolist() == [
        "CORE-000361",
        1,
        "CG0035",
        "FB0919",
        "VISIT and VISITNUM do not have a one-to-one relationship",
        "SUCCESS",
    ], "Mismatch in 'Rules Report', row 0"
