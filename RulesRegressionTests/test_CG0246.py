...
RULE_JSON_FILE = "published_rules/CG0246/CG0246.json"
DATASET_PATH = "json_datasets/CG0246/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['te.xpt', 'Trial Elements', '.', 'IGNORE_TIMESTAMP', 0, 5], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000143', 1, 'CG0246, SEND24', None, 'ETCD value is greater than 8', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
