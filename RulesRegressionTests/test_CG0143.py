...
RULE_JSON_FILE = "published_rules/CG0143/CG0143.json"
DATASET_PATH = "json_datasets/CG0143/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[1].tolist() == ['ds.xpt', 'Disposition', '.', 'IGNORE_TIMESTAMP', 0, 28], "Mismatch in 'Dataset Details', row 1"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000370', 1, 'CG0143', None, 'DM.RFICDTC is not equal to the earliest DSSTDTC, when DSTERM indicates Informed consent obtained.', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
