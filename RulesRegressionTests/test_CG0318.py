...
RULE_JSON_FILE = "published_rules/CG0318/CG0318.json"
DATASET_PATH = "json_datasets/CG0318/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['ae.xpt', 'Adverse Events', '.', 'IGNORE_TIMESTAMP', 0, 3], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[2].tolist() == ['ft.xpt', 'Functional Tests', '.', 'IGNORE_TIMESTAMP', 0, 1], "Mismatch in 'Dataset Details', row 2"

    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[3].tolist() == ['pc.xpt', 'Pharmacokinetics Concentrations', '.', 'IGNORE_TIMESTAMP', 0, 60], "Mismatch in 'Dataset Details', row 3"

    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[4].tolist() == ['pp.xpt', 'Pharmacokinetics Parameters', '.', 'IGNORE_TIMESTAMP', 0, 36], "Mismatch in 'Dataset Details', row 4"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000183', 1, 'CG0318', None, 'PP dataset is present in  study, but PC dataset is not present in study.', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
