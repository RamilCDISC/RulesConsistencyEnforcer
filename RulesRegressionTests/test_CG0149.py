...
RULE_JSON_FILE = "published_rules/CG0149/CG0149.json"
DATASET_PATH = "json_datasets/CG0149/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['dm.xpt', 'Demographics', '.', 'IGNORE_TIMESTAMP', 0, 7], "Mismatch in 'Dataset Details', row 0"

    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[1].tolist() == ['tx.xpt', 'Trial Sets', '.', 'IGNORE_TIMESTAMP', 0, 24], "Mismatch in 'Dataset Details', row 1"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000088', 1, 'CG0149, SEND25', None, 'SETCD value length is greater than 8', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
