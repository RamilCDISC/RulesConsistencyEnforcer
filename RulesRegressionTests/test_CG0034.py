...
RULE_JSON_FILE = "published_rules/CG0034/CG0034.json"
DATASET_PATH = "json_datasets/CG0034/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[3].tolist() == ['tv.xpt', 'Trial Visits', '.', 'IGNORE_TIMESTAMP', 0, 12], "Mismatch in 'Dataset Details', row 3"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000168', 1, 'CG0034', None, 'VISITNUM is not among VISITNUM in SV domain.', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
