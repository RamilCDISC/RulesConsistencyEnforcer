...
RULE_JSON_FILE = "published_rules/CG0345/CG0345.json"
DATASET_PATH = "json_datasets/CG0345/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['cm.xpt', 'Concomitant/Prior Medications', '.', 'IGNORE_TIMESTAMP', 0, 1], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000163', 1, 'CG0345', None, "--TRT is equal to 'MULTIPLE'", 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
