...
RULE_JSON_FILE = "published_rules/CG0333/CG0333.json"
DATASET_PATH = "json_datasets/CG0333/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['facm.xpt', 'Findings About CM Records', '.', 'IGNORE_TIMESTAMP', 0, 2], "Mismatch in 'Dataset Details', row 0"

    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[1].tolist() == ['cm.xpt', 'Concomitant Medications', '.', 'IGNORE_TIMESTAMP', 0, 6], "Mismatch in 'Dataset Details', row 1"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000540', 1, 'CG0333', None, 'Parent domain referenced in Findings About dataset name is not present in the study', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
