...
RULE_JSON_FILE = "published_rules/CG0077/CG0077.json"
DATASET_PATH = "json_datasets/CG0077/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['mh.xpt', 'Medical History', '.', 'IGNORE_TIMESTAMP', 0, 12], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000365', 1, 'CG0077', None, 'MHCAT is grouping all records into one generic group. If no smaller categorization can be applied, then it is not necessary to include or populate this variable.', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
