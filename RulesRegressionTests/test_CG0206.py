...
RULE_JSON_FILE = "published_rules/CG0206/CG0206.json"
DATASET_PATH = "json_datasets/CG0206/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['se.xpt', 'Subject Elements', '.', 'IGNORE_TIMESTAMP', 0, 3], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000020', 1, 'CG0206', None, "TAETORD should be null when ETCD = 'UNPLAN'.", 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
