...
RULE_JSON_FILE = "published_rules/CG0022/CG0022.json"
DATASET_PATH = "json_datasets/CG0022/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[1].tolist() == ['fa.xpt', 'Findings About Events or Interventions', '.', 'IGNORE_TIMESTAMP', 0, 12], "Mismatch in 'Dataset Details', row 1"

    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[2].tolist() == ['vs.xpt', 'Vital Signs', '.', 'IGNORE_TIMESTAMP', 0, 8], "Mismatch in 'Dataset Details', row 2"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000358', 1, 'CG0022', None, 'LNKGRP variable is not found in any of the other domains.', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
