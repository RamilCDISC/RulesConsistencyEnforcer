...
RULE_JSON_FILE = "published_rules/CG0122/CG0122.json"
DATASET_PATH = "json_datasets/CG0122/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['dm.xpt', 'Demographics', '.', 'IGNORE_TIMESTAMP', 0, 9], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000330', 1, 'CG0122', None, 'DM.ACTARM must equal DM.ARM when subject is a screen failure or not assigned to treatment', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
