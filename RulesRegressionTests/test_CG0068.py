...
RULE_JSON_FILE = "published_rules/CG0068/CG0068.json"
DATASET_PATH = "json_datasets/CG0068/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[1].tolist() == ['ds.xpt', 'Disposition', '.', 'IGNORE_TIMESTAMP', 0, 22], "Mismatch in 'Dataset Details', row 1"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000363', 1, 'CG0068', None, 'The earliest disposition date for "INFORMED CONSENT OBTAINED" is not equal to DM.RFICDTC', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
