...
RULE_JSON_FILE = "published_rules/CG0398/CG0398.json"
DATASET_PATH = "json_datasets/CG0398/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['ds.xpt', 'Disposition', '.', 'IGNORE_TIMESTAMP', 0, 6], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000586', 1, 'CG0398', None, 'Multiple records exist within the same EPOCH for a subject when DSCAT = DISPOSITION EVENT', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
