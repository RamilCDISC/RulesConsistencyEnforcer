...
RULE_JSON_FILE = "published_rules/CG0248/CG0248.json"
DATASET_PATH = "json_datasets/CG0248/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['ta.xpt', 'Trial Arms', '.', 'IGNORE_TIMESTAMP', 0, 6], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000534', 1, 'CG0248, SEND221', None, 'TAETORD is not an integer', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
