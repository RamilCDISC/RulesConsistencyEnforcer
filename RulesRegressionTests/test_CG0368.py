...
RULE_JSON_FILE = "published_rules/CG0368/CG0368.json"
DATASET_PATH = "json_datasets/CG0368/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[1].tolist() == ['ae.xpt', 'Adverse Events', '.', 'IGNORE_TIMESTAMP', 0, 6], "Mismatch in 'Dataset Details', row 1"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000581', 1, 'CG0368', 'FB8601', 'DM dataset is missing.', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
