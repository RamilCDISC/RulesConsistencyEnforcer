...
RULE_JSON_FILE = "published_rules/CG0120/CG0120.json"
DATASET_PATH = "json_datasets/CG0120/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[1].tolist() == ['dm.xpt', 'Demographics', '.', 'IGNORE_TIMESTAMP', 0, 6], "Mismatch in 'Dataset Details', row 1"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000368', 1, 'CG0120, CG0121', None, 'DM.ARM must equal Screen Failure or Not Assigned if the value is not present in TA.ARM', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
