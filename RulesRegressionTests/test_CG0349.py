...
RULE_JSON_FILE = "published_rules/CG0349/CG0349.json"
DATASET_PATH = "json_datasets/CG0349/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['lb.xpt', 'Laboratory Test Results', '.', 'IGNORE_TIMESTAMP', 0, 3], "Mismatch in 'Dataset Details', row 0"

    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[1].tolist() == ['zb.xpt', 'Conventional Test Results ', '.', 'IGNORE_TIMESTAMP', 0, 3], "Mismatch in 'Dataset Details', row 1"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000376', 1, 'CG0349', None, 'First 2 characters of prefixed variable within custom domain do not match the DOMAIN value.', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
