...
RULE_JSON_FILE = "published_rules/CG0046/CG0046.json"
DATASET_PATH = "json_datasets/CG0046/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['cm.xpt', 'Concomitant/Prior Medications', '.', 'IGNORE_TIMESTAMP', 0, 12], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000265', 1, 'CG0046', 'FB4402', '--ENRTPT is completed, but --ENTPT is not completed.', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
