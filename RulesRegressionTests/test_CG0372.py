...
RULE_JSON_FILE = "published_rules/CG0372/CG0372.json"
DATASET_PATH = "json_datasets/CG0372/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['vs.xpt', 'Vital Signs', '.', 'IGNORE_TIMESTAMP', 0, 34], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000541', 1, 'CG0372, TIG0536', None, 'IETESTCD > 8 chars or contains more than only letters, numbers, underscores, or starts with a number.', 'SKIPPED'], "Mismatch in 'Rules Report', row 0"
