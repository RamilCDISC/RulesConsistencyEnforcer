...
RULE_JSON_FILE = "published_rules/CG0412/CG0412.json"
DATASET_PATH = "json_datasets/CG0412/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['dm.xpt', 'Demographics', '.', 'IGNORE_TIMESTAMP', 0, 5], "Mismatch in 'Dataset Details', row 0"

    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[1].tolist() == ['ds.xpt', 'Disposition', '.', 'IGNORE_TIMESTAMP', 0, 25], "Mismatch in 'Dataset Details', row 1"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000515', 1, 'CG0412', None, 'DM.ACTARMCD does not equal "SCRNFAIL", "NOTASSGN" or "NOTTRT" but there are no DS records for the same subject', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
