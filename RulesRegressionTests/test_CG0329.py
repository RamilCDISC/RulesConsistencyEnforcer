...
RULE_JSON_FILE = "published_rules/CG0329/CG0329.json"
DATASET_PATH = "json_datasets/CG0329/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['te.xpt', 'Trial Elements', '.', 'IGNORE_TIMESTAMP', 0, 2], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000194', 1, 'CG0329', None, 'TEDUR cannot be null when TEENRL is null', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
