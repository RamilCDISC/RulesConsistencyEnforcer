...
RULE_JSON_FILE = "published_rules/CG0090/CG0090.json"
DATASET_PATH = "json_datasets/CG0090/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['vs.xpt', 'Vital Signs', '.', 'IGNORE_TIMESTAMP', 0, 3], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000165', 1, 'CG0090', 'FB3701', '--TPTREF is not present when --RFTDTC is present in a dataset', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
