...
RULE_JSON_FILE = "published_rules/CG0307/CG0307.json"
DATASET_PATH = "json_datasets/CG0307/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['ts.xpt', 'Trial Summary Information', '.', 'IGNORE_TIMESTAMP', 0, 29], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000179', 1, 'CG0307', 'FB0916', "TSPARM and TSPARMCD don't have a one-to-one relationship.", 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
