...
RULE_JSON_FILE = "published_rules/CG0043/CG0043.json"
DATASET_PATH = "json_datasets/CG0043/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['ae.xpt', 'Adverse Events', '.', 'IGNORE_TIMESTAMP', 0, 20], "Mismatch in 'Dataset Details', row 0"

    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[1].tolist() == ['suppae.xpt', 'Supplemental Qualifiers for AE', '.', 'IGNORE_TIMESTAMP', 0, 111], "Mismatch in 'Dataset Details', row 1"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000597', 1, 'CG0043', None, 'Missing AESMIE=Y where SUPPAE.QNAM=AESOSP', 'SKIPPED'], "Mismatch in 'Rules Report', row 0"
