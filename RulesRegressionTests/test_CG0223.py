...
RULE_JSON_FILE = "published_rules/CG0223/CG0223.json"
DATASET_PATH = "json_datasets/CG0223/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['cm.xpt', 'Concomitant/Prior Medications', '.', 'IGNORE_TIMESTAMP', 0, 4], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000139', 1, 'CG0223', None, '--ENDY is not null when either --ENDTC or DM.RFSTDTC do not contain complete values in their date portion', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
