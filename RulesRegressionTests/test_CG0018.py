...
RULE_JSON_FILE = "published_rules/CG0018/CG0018.json"
DATASET_PATH = "json_datasets/CG0018/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['suppqscq.xpt', 'Supplemental Qualifiers for QSCG', '.', 'IGNORE_TIMESTAMP', 0, 2], "Mismatch in 'Dataset Details', row 0"

    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[1].tolist() == ['suppqssw.xpt', 'Supplemental Qualifiers for QSSW', '.', 'IGNORE_TIMESTAMP', 0, 2], "Mismatch in 'Dataset Details', row 1"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000357', 1, 'CG0018', None, 'Supplemental qualifier dataset associated with a split dataset is greater than 8 characters in length', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
