...
RULE_JSON_FILE = "published_rules/CG0255/CG0255.json"
DATASET_PATH = "json_datasets/CG0255/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['ti.xpt', 'Trial Inclusion/Exclusion Criteria', '.', 'IGNORE_TIMESTAMP', 0, 62], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000145', 1, 'CG0255', None, 'IETESTCD is not unique within TIVERS', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
