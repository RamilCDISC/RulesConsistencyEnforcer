...
RULE_JSON_FILE = "published_rules/CG0359/CG0359.json"
DATASET_PATH = "json_datasets/CG0359/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['ae.xpt', 'Adverse Events', '.', 'IGNORE_TIMESTAMP', 0, 1616], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000594', 1, 'CG0359, SEND29, TIG0205, TIG0524', None, 'Variable label is not in title case.', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
