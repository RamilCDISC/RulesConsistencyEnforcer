...
RULE_JSON_FILE = "published_rules/CG0152/CG0152.json"
DATASET_PATH = "json_datasets/CG0152/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['se.xpt', 'Subject Elements', '.', 'IGNORE_TIMESTAMP', 0, 6], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000009', 1, 'CG0152, SEND124.1', None, "ELEMENT variable has a non-null value when ETCD has a value of 'UNPLAN'", 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
