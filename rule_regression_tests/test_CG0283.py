...
RULE_JSON_FILE = "published_rules/CG0283/CG0283.json"
DATASET_PATH = "json_datasets/CG0283/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['ts.xpt', 'Trial Summary', '.', 'IGNORE_TIMESTAMP', 0, 3], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Issue Summary']).iloc[0].tolist() == ['ts.xpt', 'TS', 'CORE-000504', 'Invalid date for TSVAL for TSPARMCD = DCUTDTC (Data Cutoff Date)', 1, None], "Mismatch in 'Issue Summary', row 0"
    ...
    assert clean_dataframe(new_df_dict['Issue Details']).iloc[0].tolist() == ['CORE-000504', 'Invalid date for TSVAL for TSPARMCD = DCUTDTC (Data Cutoff Date)', 'fully executable', 'ts.xpt', 'TS', None, 1, 1, 'TSPARMCD, TSVAL', 'DCUTDTC, 2023-02-30'], "Mismatch in 'Issue Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000504', 1, 'CG0283', None, 'Invalid date for TSVAL for TSPARMCD = DCUTDTC (Data Cutoff Date)', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
