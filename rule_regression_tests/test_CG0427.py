...
RULE_JSON_FILE = "published_rules/CG0427/CG0427.json"
DATASET_PATH = "json_datasets/CG0427/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['dd.xpt', 'Death Details', '.', 'IGNORE_TIMESTAMP', 0, 2], "Mismatch in 'Dataset Details', row 0"

    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[1].tolist() == ['mb.xpt', 'Microbiology Specimen', '.', 'IGNORE_TIMESTAMP', 0, 2], "Mismatch in 'Dataset Details', row 1"

    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[2].tolist() == ['mi.xpt', 'Microscopic Findings', '.', 'IGNORE_TIMESTAMP', 0, 2], "Mismatch in 'Dataset Details', row 2"

    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[3].tolist() == ['ms.xpt', 'Microbiology Susceptibility', '.', 'IGNORE_TIMESTAMP', 0, 2], "Mismatch in 'Dataset Details', row 3"

    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[4].tolist() == ['oe.xpt', 'Ophthalmic Examinations', '.', 'IGNORE_TIMESTAMP', 0, 2], "Mismatch in 'Dataset Details', row 4"

    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[5].tolist() == ['ur.xpt', 'Urinary System Findings', '.', 'IGNORE_TIMESTAMP', 0, 2], "Mismatch in 'Dataset Details', row 5"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000101', 1, 'CG0427, TIG0581', None, 'Missing --STRESC value, when --RESCAT value is populated.', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
