...
RULE_JSON_FILE = "published_rules/CG0017/CG0017.json"
DATASET_PATH = "json_datasets/CG0017/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['qscg.xpt', 'Clinical Global Impressions', '.', 'IGNORE_TIMESTAMP', 0, 1], "Mismatch in 'Dataset Details', row 0"

    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[1].tolist() == ['qspg.xpt', 'Patient Global Impressions', '.', 'IGNORE_TIMESTAMP', 0, 1], "Mismatch in 'Dataset Details', row 1"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000510', 1, 'CG0017', None, 'Split dataset name is not 3 or 4 characters in length', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
