...
RULE_JSON_FILE = "published_rules/CG0075/CG0075.json"
DATASET_PATH = "json_datasets/CG0075/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['dm.xpt', 'Demographics', '.', 'IGNORE_TIMESTAMP', 0, 2], "Mismatch in 'Dataset Details', row 0"

    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[1].tolist() == ['dv.xpt', 'Protocol Deviations', '.', 'IGNORE_TIMESTAMP', 0, 4], "Mismatch in 'Dataset Details', row 1"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000086', 1, 'CG0075', None, 'DVSTDTC is earlier than RFICDTC in DM.', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
