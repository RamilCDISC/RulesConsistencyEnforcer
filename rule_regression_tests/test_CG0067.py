...
RULE_JSON_FILE = "published_rules/CG0067/CG0067.json"
DATASET_PATH = "json_datasets/CG0067/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['ds.xpt', 'Disposition', '.', 'IGNORE_TIMESTAMP', 0, 13], "Mismatch in 'Dataset Details', row 0"

    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[1].tolist() == ['ss.xpt', 'Subject Status', '.', 'IGNORE_TIMESTAMP', 0, 4], "Mismatch in 'Dataset Details', row 1"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000251', 1, 'CG0067', None, 'DSDECOD is not "DEATH", When SS.SSSTRESC = "DEAD".', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
