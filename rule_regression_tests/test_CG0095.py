...
RULE_JSON_FILE = "published_rules/CG0095/CG0095.json"
DATASET_PATH = "json_datasets/CG0095/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['oe.xpt', 'Ophthamology', '.', 'IGNORE_TIMESTAMP', 0, 285], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000079', 1, 'CG0095', None, '--LAT is present when --LOC is not present.', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
