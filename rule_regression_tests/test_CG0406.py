...
RULE_JSON_FILE = "published_rules/CG0406/CG0406.json"
DATASET_PATH = "json_datasets/CG0406/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['oe.xpt', 'Opthalmic Examinations', '.', 'IGNORE_TIMESTAMP', 0, 21], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000199', 1, 'CG0406, SEND64, TIG0563', None, 'Value length of --TEST > 40.', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
