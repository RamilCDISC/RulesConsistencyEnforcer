...
RULE_JSON_FILE = "published_rules/CG0364/CG0364.json"
DATASET_PATH = "json_datasets/CG0364/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['relsub.xpt', 'Related to Subjects', '.', 'IGNORE_TIMESTAMP', 0, 6], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000232', 1, 'CG0364', None, 'When RSUBJID is populated, RUSBJID cannot equal POOLID', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
