...
RULE_JSON_FILE = "published_rules/CG0241/CG0241.json"
DATASET_PATH = "json_datasets/CG0241/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['ft.xpt', 'Functional Tests', '.', 'IGNORE_TIMESTAMP', 0, 6], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000142', 1, 'CG0241', None, '--ELTM is not the same value across records with the same values of DOMAIN, VISITNUM, --TPTREF, and --TPTNUM', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
