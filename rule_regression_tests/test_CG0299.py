...
RULE_JSON_FILE = "published_rules/CG0299/CG0299.json"
DATASET_PATH = "json_datasets/CG0299/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['tr.xpt', 'Tumor/Lesion Results', '.', 'IGNORE_TIMESTAMP', 0, 27], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Issue Summary']).iloc[0].tolist() == ['tr.xpt', 'TR', 'CORE-000003', 'TRLOC is present when TRLOC is not included in the TR domain.', 1, None], "Mismatch in 'Issue Summary', row 0"
    ...
    assert clean_dataframe(new_df_dict['Issue Details']).iloc[0].tolist() == ['CORE-000003', 'TRLOC is present when TRLOC is not included in the TR domain.', 'fully executable', 'tr.xpt', 'TR', None, None, None, 'TRLOC', None], "Mismatch in 'Issue Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000003', 1, 'CG0299', None, 'TRLOC is present when TRLOC is not included in the TR domain.', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
