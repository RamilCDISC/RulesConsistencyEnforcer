...
RULE_JSON_FILE = "published_rules/CG0202/CG0202.json"
DATASET_PATH = "json_datasets/CG0202/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['suppae.xpt', 'Supplemental Qualifiers for Adverse Events', '.', 'IGNORE_TIMESTAMP', 0, 8], "Mismatch in 'Dataset Details', row 0"

    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[1].tolist() == ['suppec.xpt', 'Supplemental Qualifiers for Exposure as Collected', '.', 'IGNORE_TIMESTAMP', 0, 3], "Mismatch in 'Dataset Details', row 1"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000375', 1, 'CG0202', None, 'QORIG = ASSIGNED but QEVAL is not populated', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
