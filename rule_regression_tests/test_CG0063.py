...
RULE_JSON_FILE = "published_rules/CG0063/CG0063.json"
DATASET_PATH = "json_datasets/CG0063/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['ds.xpt', 'Disposition', '.', 'IGNORE_TIMESTAMP', 0, 7], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000329', 1, 'CG0063', None, 'Multiple records within same USUBJID exist with DSCAT = DISPOSITION EVENT but EPOCH is empty', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
