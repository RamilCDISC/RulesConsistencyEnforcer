...
RULE_JSON_FILE = "published_rules/CG0428/CG0428.json"
DATASET_PATH = "json_datasets/CG0428/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[2].tolist() == ['ce.xpt', 'Clinical Events', '.', 'IGNORE_TIMESTAMP', 0, 2], "Mismatch in 'Dataset Details', row 2"

    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[3].tolist() == ['eg.xpt', 'ECG Test Results', '.', 'IGNORE_TIMESTAMP', 0, 2], "Mismatch in 'Dataset Details', row 3"

    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[4].tolist() == ['lb.xpt', 'Laboratory Test Results', '.', 'IGNORE_TIMESTAMP', 0, 2], "Mismatch in 'Dataset Details', row 4"

    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[5].tolist() == ['pc.xpt', 'Pharmacokinetics Concentrations', '.', 'IGNORE_TIMESTAMP', 0, 2], "Mismatch in 'Dataset Details', row 5"

    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[6].tolist() == ['pp.xpt', 'Pharmacokinetics Parameters', '.', 'IGNORE_TIMESTAMP', 0, 2], "Mismatch in 'Dataset Details', row 6"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000102', 1, 'CG0428', None, '--TOXGR should not be null when --TOX is populated.', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
