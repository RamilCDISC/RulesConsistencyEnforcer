...
RULE_JSON_FILE = "published_rules/CG0528/CG0528.json"
DATASET_PATH = "json_datasets/CG0528/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['suppdm.xpt', 'Supplemental Qualifiers for Demographics', '.', 'IGNORE_TIMESTAMP', 0, 4], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000211', 1, 'CG0528', None, 'Population flag is present in SUPPDM', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
