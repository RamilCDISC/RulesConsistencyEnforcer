...
RULE_JSON_FILE = "published_rules/CG0514/CG0514.json"
DATASET_PATH = "json_datasets/CG0514/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[1].tolist() == ['ta.xpt', 'Trial Arms', '.', 'IGNORE_TIMESTAMP', 0, 8], "Mismatch in 'Dataset Details', row 1"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000209', 1, 'CG0514', None, 'ACTARM value in DM dataset is not among the values of ARM variable in the TA dataset. This is allowed only in a multistage study with incomplete ARM assignment. Please confirm if your study is a multistage assignment study.', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
