...
RULE_JSON_FILE = "published_rules/CG0362/CG0362.json"
DATASET_PATH = "json_datasets/CG0362/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['relsub.xpt', 'Related Subjects ', '.', 'IGNORE_TIMESTAMP', 0, 6], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000230', 1, 'CG0362', None, 'USUBJID must be null when POOLID is populated', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
