...
RULE_JSON_FILE = "published_rules/CG0093/CG0093.json"
DATASET_PATH = "json_datasets/CG0093/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['ec.xpt', 'Exposure as Collected', '.', 'IGNORE_TIMESTAMP', 0, 1590], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000080', 1, 'CG0093', None, '--TPTREF is present when --ELTM, --TPTNUM, and --TPT are not present.', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
