...
RULE_JSON_FILE = "published_rules/CG0413/CG0413.json"
DATASET_PATH = "json_datasets/CG0413/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['lb.xpt', 'Laboratory Test Results', '.', 'IGNORE_TIMESTAMP', 0, 1], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000598', 1, 'CG0413, TIG0568', None, 'Dataset name does not begin with DOMAIN value', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
