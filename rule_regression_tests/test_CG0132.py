...
RULE_JSON_FILE = "published_rules/CG0132/CG0132.json"
DATASET_PATH = "json_datasets/CG0132/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[1].tolist() == ['ss.xpt', 'Subject Status', '.', 'IGNORE_TIMESTAMP', 0, 3], "Mismatch in 'Dataset Details', row 1"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000008', 1, 'CG0132', None, 'DTHFL is not "Y", when SS.SSSTRESC equals "DEAD"', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
