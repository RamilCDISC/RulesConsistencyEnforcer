...
RULE_JSON_FILE = "published_rules/CG0346/CG0346.json"
DATASET_PATH = "json_datasets/CG0346/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['dv.xpt', 'Protocol Deviations', '.', 'IGNORE_TIMESTAMP', 0, 1], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000164', 1, 'CG0346', None, "--TERM is equal to 'MULTIPLE'", 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
