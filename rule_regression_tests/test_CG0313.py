...
RULE_JSON_FILE = "published_rules/CG0313/CG0313.json"
DATASET_PATH = "json_datasets/CG0313/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[1].tolist() == ['lb.xpt', 'Laboratory Test Results', '.', 'IGNORE_TIMESTAMP', 0, 6], "Mismatch in 'Dataset Details', row 1"

    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[2].tolist() == ['ec.xpt', 'Exposure as Collected', '.', 'IGNORE_TIMESTAMP', 0, 3], "Mismatch in 'Dataset Details', row 2"

    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[3].tolist() == ['fa.xpt', 'Findings About', '.', 'IGNORE_TIMESTAMP', 0, 5], "Mismatch in 'Dataset Details', row 3"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000585', 1, 'CG0313', None, 'One of following variables is not present in the dataset; STUDYID, DOMAIN, USUBJID (or POOLID) or --SEQ.', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
