...
RULE_JSON_FILE = "published_rules/CG0535/CG0535.json"
DATASET_PATH = "json_datasets/CG0535/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['ds.xpt', 'Subject Disposition', '.', 'IGNORE_TIMESTAMP', 0, 5], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000212', 1, 'CG0535', None, "DSSCAT is not present when subject has more than one record per Epoch with DSCAT = 'DISPOSITION EVENT'.", 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
