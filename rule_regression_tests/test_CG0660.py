...
RULE_JSON_FILE = "published_rules/CG0660/CG0660.json"
DATASET_PATH = "json_datasets/CG0660/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['ft.xpt', 'Functional Tests', '.', 'IGNORE_TIMESTAMP', 0, 10], "Mismatch in 'Dataset Details', row 0"

    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[1].tolist() == ['qs.xpt', 'Questionnaires', '.', 'IGNORE_TIMESTAMP', 0, 12], "Mismatch in 'Dataset Details', row 1"

    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[2].tolist() == ['rs.xpt', 'Disease Response and Clin Classification', '.', 'IGNORE_TIMESTAMP', 0, 6], "Mismatch in 'Dataset Details', row 2"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000032', 1, 'CG0660', None, '--EVALID is present.  --EVALID must not be used to model QRS data.', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
