...
RULE_JSON_FILE = "published_rules/CG0284/CG0284.json"
DATASET_PATH = "json_datasets/CG0284/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['ts.xpt', 'Trial Summary Information', '.', 'IGNORE_TIMESTAMP', 0, 30], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000587', 1, 'CG0284', None, 'TSVAL must be an integer greater than zero when TSPARMCD is NARMS', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
