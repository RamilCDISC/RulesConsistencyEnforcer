...
RULE_JSON_FILE = "published_rules/CG0175/CG0175.json"
DATASET_PATH = "json_datasets/CG0175/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['ie.xpt', 'Inclusion/Exclusion Criteria Not Met', '.', 'IGNORE_TIMESTAMP', 0, 1], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000011', 1, 'CG0175', None, 'IEORRES = N for an exclusion criteria. Only exclusion criteria not met should be included in IE domain.', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
