...
RULE_JSON_FILE = "published_rules/CG0538/CG0538.json"
DATASET_PATH = "json_datasets/CG0538/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['ds.xpt', 'Subject Disposition', '.', 'IGNORE_TIMESTAMP', 0, 2], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000214', 1, 'CG0538', None, "There is more than 1 record per subject has DSSCAT = 'STUDY PARTICIPATION'", 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
