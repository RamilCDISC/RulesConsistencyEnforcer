...
RULE_JSON_FILE = "published_rules/CG0656/CG0656.json"
DATASET_PATH = "json_datasets/CG0656/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['sv.xpt', 'Subject Visits', '.', 'IGNORE_TIMESTAMP', 0, 10], "Mismatch in 'Dataset Details', row 0"

    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[1].tolist() == ['tv.xpt', 'Trial Visits', '.', 'IGNORE_TIMESTAMP', 0, 12], "Mismatch in 'Dataset Details', row 1"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000040', 1, 'CG0656', None, 'VISITNUM for unplanned visit is present in TV.', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
