...
RULE_JSON_FILE = "published_rules/CG0518/CG0518.json"
DATASET_PATH = "json_datasets/CG0518/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['dm.xpt', 'Demographics', '.', 'IGNORE_TIMESTAMP', 0, 18], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000047', 1, 'CG0518', None, 'ARM value in DM dataset is not among the values of ARM variable in TA dataset. This is allowed only in a multistage study with incomplete ARM assignment. Please confirm if your study is a multistage assignment study', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
