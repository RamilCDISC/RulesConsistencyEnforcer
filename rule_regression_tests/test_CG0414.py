...
RULE_JSON_FILE = "published_rules/CG0414/CG0414.json"
DATASET_PATH = "json_datasets/CG0414/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['se.xpt', 'Subject Elements', '.', 'IGNORE_TIMESTAMP', 0, 4], "Mismatch in 'Dataset Details', row 0"

    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[1].tolist() == ['te.xpt', 'Trial Elements', '.', 'IGNORE_TIMESTAMP', 0, 5], "Mismatch in 'Dataset Details', row 1"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000173', 1, 'CG0414', None, "ETCD is not equal to 'UNPLAN' and not equal to TE.ETCD", 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
