...
RULE_JSON_FILE = "published_rules/CG0066/CG0066.json"
DATASET_PATH = "json_datasets/CG0066/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['ds.xpt', 'Disposition', '.', 'IGNORE_TIMESTAMP', 0, 4], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000322', 1, 'CG0066', None, 'DSCAT equals PROTOCOL MILESTONE but DSTERM is not equal to DSDECOD', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
