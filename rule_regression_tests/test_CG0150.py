...
RULE_JSON_FILE = "published_rules/CG0150/CG0150.json"
DATASET_PATH = "json_datasets/CG0150/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['dm.xpt', 'Demographics', '.', 'IGNORE_TIMESTAMP', 0, 20], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000186', 1, 'CG0150', None, 'SUBJID is not unique within study', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
