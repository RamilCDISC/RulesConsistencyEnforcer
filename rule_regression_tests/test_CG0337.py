...
RULE_JSON_FILE = "published_rules/CG0337/CG0337.json"
DATASET_PATH = "json_datasets/CG0337/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['ae.xpt', 'Adverse Events', '.', 'IGNORE_TIMESTAMP', 0, 15], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000196', 1, 'CG0337', None, '--CAT is equal to --DECOD.', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
