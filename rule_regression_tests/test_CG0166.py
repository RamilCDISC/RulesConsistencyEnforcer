...
RULE_JSON_FILE = "published_rules/CG0166/CG0166.json"
DATASET_PATH = "json_datasets/CG0166/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['co.xpt', 'Comments', '.', 'IGNORE_TIMESTAMP', 0, 8], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000017', 1, 'CG0166', None, 'RDOMAIN has a missing value when IDVARVAL has a non-missing value', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
