...
RULE_JSON_FILE = "published_rules/CG0545/CG0545.json"
DATASET_PATH = "json_datasets/CG0545/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['sm.xpt', 'Subject Disease Milestones', '.', 'IGNORE_TIMESTAMP', 0, 4], "Mismatch in 'Dataset Details', row 0"

    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[1].tolist() == ['tm.xpt', 'Trial Disease Milestones', '.', 'IGNORE_TIMESTAMP', 0, 2], "Mismatch in 'Dataset Details', row 1"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000216', 1, 'CG0545', None, "MIDSTYPE is not unique within subject when MIDSTYPE = TM.MIDSTYPE and TM.TMRPT = 'N'", 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
