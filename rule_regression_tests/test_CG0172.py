...
RULE_JSON_FILE = "published_rules/CG0172/CG0172.json"
DATASET_PATH = "json_datasets/CG0172/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['ds.xpt', 'Disposition', '.', 'IGNORE_TIMESTAMP', 0, 23], "Mismatch in 'Dataset Details', row 0"

    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[1].tolist() == ['ss.xpt', 'Subject Status', '.', 'IGNORE_TIMESTAMP', 0, 10], "Mismatch in 'Dataset Details', row 1"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000178', 1, 'CG0172', None, "SSSTRESC = 'DEAD', but SSDTC < max DS.DSSTDTC.", 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
