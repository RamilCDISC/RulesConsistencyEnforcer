...
RULE_JSON_FILE = "published_rules/CG0234/CG0234.json"
DATASET_PATH = "json_datasets/CG0234/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[1].tolist() == ['mh.xpt', 'Medical History', '.', 'IGNORE_TIMESTAMP', 0, 14], "Mismatch in 'Dataset Details', row 1"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000324', 1, 'CG0234', None, "--ENRTPT is not in ('BEFORE', 'COINCIDENT', 'ONGOING', 'UNKNOWN')", 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
