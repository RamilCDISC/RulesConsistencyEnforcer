...
RULE_JSON_FILE = "published_rules/CG0091/CG0091.json"
DATASET_PATH = "json_datasets/CG0091/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['vs.xpt', 'Vital Signs', '.', 'IGNORE_TIMESTAMP', 0, 2], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000166', 1, 'CG0091, TIG0356', None, '--TPT is not present when --TPTNUM is present in a dataset', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
