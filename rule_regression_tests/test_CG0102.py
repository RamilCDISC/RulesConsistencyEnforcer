...
RULE_JSON_FILE = "published_rules/CG0102/CG0102.json"
DATASET_PATH = "json_datasets/CG0102/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['ex.xpt', 'Exposure', '.', 'IGNORE_TIMESTAMP', 0, 12], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000005', 1, 'CG0102', None, 'EXTRT is PLACEBO, but EXDOSE is not equal to 0.', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
