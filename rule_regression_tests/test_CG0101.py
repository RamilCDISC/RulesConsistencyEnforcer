...
RULE_JSON_FILE = "published_rules/CG0101/CG0101.json"
DATASET_PATH = "json_datasets/CG0101/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['ec.xpt', 'Exposure as Collected', '.', 'IGNORE_TIMESTAMP', 0, 16], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000004', 1, 'CG0101', None, 'ECOCCUR indicates dose was not given, but ECDOSE is not blank or has a value less than or equal to 0', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
