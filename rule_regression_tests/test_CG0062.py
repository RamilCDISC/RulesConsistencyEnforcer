...
RULE_JSON_FILE = "published_rules/CG0062/CG0062.json"
DATASET_PATH = "json_datasets/CG0062/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['pr.xpt', 'Procedures', '.', 'IGNORE_TIMESTAMP', 0, 8], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000258', 1, 'CG0062', None, '--STRTPT is present in dataset, but --STTPT is not present in dataset.', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
