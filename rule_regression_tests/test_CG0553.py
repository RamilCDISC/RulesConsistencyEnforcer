...
RULE_JSON_FILE = "published_rules/CG0553/CG0553.json"
DATASET_PATH = "json_datasets/CG0553/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['vs.xpt', 'Vital Signs', '.', 'IGNORE_TIMESTAMP', 0, 67], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000083', 1, 'CG0553', None, '--ORRES is missing even though --LOBXFL= "Y" and either --DRVFL is not present in dataset or is not equal to "Y"', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
