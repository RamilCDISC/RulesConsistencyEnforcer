...
RULE_JSON_FILE = "published_rules/CG0325/CG0325.json"
DATASET_PATH = "json_datasets/CG0325/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['te.xpt', 'Trial Elements', '.', 'IGNORE_TIMESTAMP', 0, 4], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000580', 1, 'CG0325', None, 'The combination of TESTRL, TEENRL, and TEDUR is not unique for each ETCD', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
