...
RULE_JSON_FILE = "published_rules/CG0501/CG0501.json"
DATASET_PATH = "json_datasets/CG0501/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['mh.xpt', 'Medical History', '.', 'IGNORE_TIMESTAMP', 0, 2], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000297', 1, 'CG0501', None, 'TM dataset is missing when MIDS variable is present in one of the dataset.', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
