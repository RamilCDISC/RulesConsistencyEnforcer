...
RULE_JSON_FILE = "published_rules/CG0207/CG0207.json"
DATASET_PATH = "json_datasets/CG0207/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['se.xpt', 'Subject Elements', '.', 'IGNORE_TIMESTAMP', 0, 13], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000352', 1, 'CG0207, SEND126', None, 'SEENDTC is not equal to SESTDTC of the next element.', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
