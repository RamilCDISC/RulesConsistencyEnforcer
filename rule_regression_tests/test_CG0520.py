...
RULE_JSON_FILE = "published_rules/CG0520/CG0520.json"
DATASET_PATH = "json_datasets/CG0520/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['dm.xpt', 'Demographics', '.', 'IGNORE_TIMESTAMP', 0, 4], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000121', 1, 'CG0520', None, 'Value for ARMNRS is populated, when both ARMCD and ACTARMCD values are populated', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
