...
RULE_JSON_FILE = "published_rules/CG0465/CG0465.json"
DATASET_PATH = "json_datasets/CG0465/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['suppec.xpt', 'Supplemental Qualifiers for Exposure as Collected', '.', 'IGNORE_TIMESTAMP', 0, 4], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000218', 1, 'CG0465, SEND118, TIG0055, TIG0595', None, 'IDVAR is empty but IDVARVAL is not empty', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
