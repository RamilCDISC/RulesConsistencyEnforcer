...
RULE_JSON_FILE = "published_rules/CG0218/CG0218.json"
DATASET_PATH = "json_datasets/CG0218/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['se.xpt', 'Subject Elements', '.', 'IGNORE_TIMESTAMP', 0, 8], "Mismatch in 'Dataset Details', row 0"

    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[1].tolist() == ['sv.xpt', 'Subject Visits', '.', 'IGNORE_TIMESTAMP', 0, 251], "Mismatch in 'Dataset Details', row 1"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000097', 1, 'CG0218', None, "EPOCH values don't match between Subject Visits (SV) and Subject Elements (SE) datasets.", 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
