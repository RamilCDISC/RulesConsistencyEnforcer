...
RULE_JSON_FILE = "published_rules/CG0422/CG0422.json"
DATASET_PATH = "json_datasets/CG0422/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['eg.xpt', 'ECG Test Results', '.', 'IGNORE_TIMESTAMP', 0, 3], "Mismatch in 'Dataset Details', row 0"

    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[1].tolist() == ['fa.xpt', 'Findings About Events or Interventions', '.', 'IGNORE_TIMESTAMP', 0, 2], "Mismatch in 'Dataset Details', row 1"
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[3].tolist() == ['lb.xpt', 'Laboratory Test Results', '.', 'IGNORE_TIMESTAMP', 0, 2], "Mismatch in 'Dataset Details', row 3"

    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[4].tolist() == ['vs.xpt', 'Vital Signs', '.', 'IGNORE_TIMESTAMP', 0, 2], "Mismatch in 'Dataset Details', row 4"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000099', 1, 'CG0422, TIG0577', None, 'Value for --STAT is populated, when --ORRES is populated', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
