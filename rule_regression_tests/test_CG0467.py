...
RULE_JSON_FILE = "published_rules/CG0467/CG0467.json"
DATASET_PATH = "json_datasets/CG0467/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['lb.xpt', 'Laboratory Test Results', '.', 'IGNORE_TIMESTAMP', 0, 18], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000207', 1, 'CG0467, SEND78, TIG0276, TIG0597', None, '--STDTC is present in a Findings general observation class', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
