...
RULE_JSON_FILE = "published_rules/CG0568/CG0568.json"
DATASET_PATH = "json_datasets/CG0568/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['ex.xpt', 'Exposure', '.', 'IGNORE_TIMESTAMP', 0, 1], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000326', 1, 'CG0568', None, 'EXMETHOD is not to be used with human clinical trials', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
