...
RULE_JSON_FILE = "published_rules/CG0619/CG0619.json"
DATASET_PATH = "json_datasets/CG0619/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['lb.xpt', 'Laboratory', '.', 'IGNORE_TIMESTAMP', 0, 2], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000116', 1, 'CG0619, TIG0660', None, "--SPCUFL is not null or equal to 'N'", 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
