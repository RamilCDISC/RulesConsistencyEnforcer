...
RULE_JSON_FILE = "published_rules/CG0032/CG0032.json"
DATASET_PATH = "json_datasets/CG0032/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Issue Summary']).iloc[0].tolist() == ['lb.xpt', 'LB', 'CORE-000249', 'Visit Day cannot be found in Trial Visit (TV) domain', 1, None], "Mismatch in 'Issue Summary', row 0"
    ...
    assert clean_dataframe(new_df_dict['Issue Details']).iloc[0].tolist() == ['CORE-000249', 'Visit Day cannot be found in Trial Visit (TV) domain', 'fully executable', 'lb.xpt', 'LB', 'CDISC-TEST-001', 1, 1, 'VISITDY, VISITNUM', '-14, 100'], "Mismatch in 'Issue Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000249', 1, 'CG0032', None, 'Visit Day cannot be found in Trial Visit (TV) domain', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
