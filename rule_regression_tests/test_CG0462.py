...
RULE_JSON_FILE = "published_rules/CG0462/CG0462.json"
DATASET_PATH = "json_datasets/CG0462/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['ec.xpt', 'Exposure as Collected', '.', 'IGNORE_TIMESTAMP', 0, 1], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000217', 1, 'CG0462', None, "ECDOSE is empty, ECOCCUR is not equal to 'N' and ECSTAT is empty, but ECDOSTXT is also empty.", 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
