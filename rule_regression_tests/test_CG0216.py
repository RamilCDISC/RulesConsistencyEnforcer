...
RULE_JSON_FILE = "published_rules/CG0216/CG0216.json"
DATASET_PATH = "json_datasets/CG0216/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['sv.xpt', 'Subject Visits', '.', 'IGNORE_TIMESTAMP', 0, 8], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000501', 1, 'CG0216', None, 'VISITDY is populated when SVUPDES is also populated.', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
