...
RULE_JSON_FILE = "published_rules/CG0109/CG0109.json"
DATASET_PATH = "json_datasets/CG0109/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['dm.xpt', 'Demographics', '.', 'IGNORE_TIMESTAMP', 0, 6], "Mismatch in 'Dataset Details', row 0"

    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[1].tolist() == ['ex.xpt', 'Exposure', '.', 'IGNORE_TIMESTAMP', 0, 3], "Mismatch in 'Dataset Details', row 1"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000366', 1, 'CG0109', None, 'No EX record exists for a subject, but ACTARMCD does not equal SCRNFAIL, NOTASSGN, or NOTTRT in DM', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
