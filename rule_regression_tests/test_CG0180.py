...
RULE_JSON_FILE = "published_rules/CG0180/CG0180.json"
DATASET_PATH = "json_datasets/CG0180/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['lb.xpt', 'Laboratory Test Results', '.', 'IGNORE_TIMESTAMP', 0, 2], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Issue Summary']).iloc[0].tolist() == ['lb.xpt', 'LB', 'CORE-000290', 'LBORRES is not a continuous measurement or Empty, when LBORNRLO is not empty.', 2, None], "Mismatch in 'Issue Summary', row 0"
    ...
    assert clean_dataframe(new_df_dict['Issue Details']).iloc[0].tolist() == ['CORE-000290', 'LBORRES is not a continuous measurement or Empty, when LBORNRLO is not empty.', 'partially executable - possible underreporting', 'lb.xpt', 'LB', 'ALB', 1, 1, 'LBORNRLO, LBORRES', '3.5, NEGATIVE'], "Mismatch in 'Issue Details', row 0"

    assert clean_dataframe(new_df_dict['Issue Details']).iloc[1].tolist() == ['CORE-000290', 'LBORRES is not a continuous measurement or Empty, when LBORNRLO is not empty.', 'partially executable - possible underreporting', 'lb.xpt', 'LB', 'Prot', 2, 2, 'LBORNRLO, LBORRES', '40, POSITIVE'], "Mismatch in 'Issue Details', row 1"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000290', 1, 'CG0180', None, 'LBORRES is not a continuous measurement or Empty, when LBORNRLO is not empty.', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
