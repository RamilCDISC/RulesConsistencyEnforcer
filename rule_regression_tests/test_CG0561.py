...
RULE_JSON_FILE = "published_rules/CG0561/CG0561.json"
DATASET_PATH = "json_datasets/CG0561/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['pe.xpt', 'Physical Examination', '.', 'IGNORE_TIMESTAMP', 0, 7], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000082', 1, 'CG0561', None, 'PESTRESC value is not missing when PEORRES value is missing', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
