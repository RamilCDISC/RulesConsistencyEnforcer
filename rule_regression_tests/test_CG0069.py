...
RULE_JSON_FILE = "published_rules/CG0069/CG0069.json"
DATASET_PATH = "json_datasets/CG0069/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[1].tolist() == ['ds.xpt', 'Disposition', '.', 'IGNORE_TIMESTAMP', 0, 26], "Mismatch in 'Dataset Details', row 1"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000034', 1, 'CG0069', None, 'DSSTDTC does not equal DM.DTHDTC, when DSDECOD equals "DEATH".', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
