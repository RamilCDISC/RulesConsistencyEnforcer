...
RULE_JSON_FILE = "published_rules/CG0085/CG0085.json"
DATASET_PATH = "json_datasets/CG0085/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['ae.xpt', 'Adverse Events', '.', 'IGNORE_TIMESTAMP', 0, 18], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000260', 1, 'CG0085', None, "--PRESP does not equal 'Y' or is not empty.", 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
