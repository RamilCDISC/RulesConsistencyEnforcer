...
RULE_JSON_FILE = "published_rules/CG0421/CG0421.json"
DATASET_PATH = "json_datasets/CG0421/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[1].tolist() == ['ce.xpt', 'Clinical Events', '.', 'IGNORE_TIMESTAMP', 0, 3], "Mismatch in 'Dataset Details', row 1"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000241', 1, 'CG0421', None, '--ENRF is populated when --OCCUR = "N"', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
