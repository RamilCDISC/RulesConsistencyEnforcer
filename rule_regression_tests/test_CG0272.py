...
RULE_JSON_FILE = "published_rules/CG0272/CG0272.json"
DATASET_PATH = "json_datasets/CG0272/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['ts.xpt', 'Trial Summary', '.', 'IGNORE_TIMESTAMP', 0, 51], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000473', 1, 'CG0272', None, 'A record exists in TS where TSPARMCD = HLTSUBJI and TSVAL = Y but TSVAL for TSPARMCD = TDIGRP is not empty or does not equal HEALTHY SUBJECTS.', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
