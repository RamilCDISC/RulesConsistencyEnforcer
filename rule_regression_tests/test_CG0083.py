...
RULE_JSON_FILE = "published_rules/CG0083/CG0083.json"
DATASET_PATH = "json_datasets/CG0083/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['ae.xpt', 'Adverse Events', '.', 'IGNORE_TIMESTAMP', 0, 74], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000184', 1, 'CG0083', None, 'There is not a one-to-one relationship between --BODSYS and --BDSYSCD', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
