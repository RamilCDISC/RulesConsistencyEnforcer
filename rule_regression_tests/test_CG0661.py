...
RULE_JSON_FILE = "published_rules/CG0661/CG0661.json"
DATASET_PATH = "json_datasets/CG0661/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['vs.xpt', 'Vital Signs', '.', 'IGNORE_TIMESTAMP', 0, 10], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000029', 1, 'CG0661, TIG0697', None, '--TPTNUM exists in a dataset, but --TPT does not exist. When time points are represented in SDTMIG domains, both --TPT and --TPTNUM must be used.', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
