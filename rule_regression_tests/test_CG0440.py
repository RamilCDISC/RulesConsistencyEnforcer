...
RULE_JSON_FILE = "published_rules/CG0440/CG0440.json"
DATASET_PATH = "json_datasets/CG0440/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['ts.xpt', 'Trial Summary', '.', 'IGNORE_TIMESTAMP', 0, 57], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000340', 1, 'CG0440', None, 'TSPARMCD = PLANSUB but the value provided in TSVAL is not a positive integer.', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
