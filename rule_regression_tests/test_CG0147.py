...
RULE_JSON_FILE = "published_rules/CG0147/CG0147.json"
DATASET_PATH = "json_datasets/CG0147/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['dm.xpt', 'Demographics', '.', 'IGNORE_TIMESTAMP', 0, 1], "Mismatch in 'Dataset Details', row 0"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000238', 1, 'CG0147', None, 'RFXENDTC does not equal the latest value of EX.EXSTDTC or EX.EXENDTC', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
