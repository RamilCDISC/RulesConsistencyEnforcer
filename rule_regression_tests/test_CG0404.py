...
RULE_JSON_FILE = "published_rules/CG0404/CG0404.json"
DATASET_PATH = "json_datasets/CG0404/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['cm.xpt', 'Concomitant/Prior Medications', '.', 'IGNORE_TIMESTAMP', 0, 3], "Mismatch in 'Dataset Details', row 0"

    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[1].tolist() == ['mh.xpt', 'Medical History', '.', 'IGNORE_TIMESTAMP', 0, 4], "Mismatch in 'Dataset Details', row 1"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000118', 1, 'CG0404, TIG0562', None, '--STAT is not present in dataset when --PRESP is equal to "Y" and --OCCUR is blank', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
