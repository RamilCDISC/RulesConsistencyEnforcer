...
RULE_JSON_FILE = "published_rules/CG0225/CG0225.json"
DATASET_PATH = "json_datasets/CG0225/converted_dataset.json"
...
def test_excel():
    ...
    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[0].tolist() == ['tv.xpt', 'Trial Visits', '.', 'IGNORE_TIMESTAMP', 0, 14], "Mismatch in 'Dataset Details', row 0"

    assert clean_dataframe(new_df_dict['Dataset Details']).iloc[1].tolist() == ['sv.xpt', 'Subject Visits', '.', 'IGNORE_TIMESTAMP', 0, 6], "Mismatch in 'Dataset Details', row 1"
    ...
    assert clean_dataframe(new_df_dict['Rules Report']).iloc[0].tolist() == ['CORE-000140', 1, 'CG0225', None, 'VISITDY is not null when VISITNUM is not in TV.VISITNUM', 'SUCCESS'], "Mismatch in 'Rules Report', row 0"
